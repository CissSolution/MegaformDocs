...
export class UmbAuthContext extends UmbContextBase {
    ...
    constructor(host, serverUrl, backofficePath, isBypassed) {
        ...
        this.#isAuthorized = new UmbBooleanState(false);
        // Timeout is different from `isAuthorized` because it can occur repeatedly
        this.#isTimeout = new Subject();
        this.#isInitialized = new ReplaySubject(1);
        /**
         ...
         */
        this.timeoutSignal = this.#isTimeout.asObservable();
        this.#isBypassed = isBypassed;
        this.#serverUrl = serverUrl;
        this.#backofficePath = backofficePath;
        this.#authFlow = new UmbAuthFlow(serverUrl, this.getRedirectUrl(), this.getPostLogoutRedirectUrl(), this.#isTimeout);
        // Observe the authorization signal and close the auth window
        this.observe(this.authorizationSignal, () => {
     ...
     */
    async makeAuthorizationRequest(identityProvider = 'Umbraco', redirect, usernameHint, manifest) {
        ...
        const popupFeatures = manifest?.meta?.behavior?.popupFeatures ??
     ...
     */
    getAuthProviders(extensionsRegistry) {
        return this.#isInitialized.pipe(switchMap(() => extensionsRegistry.byType('authProvider')));
    }
    /**
     ...
     */
    unlinkLogin(providerName, providerKey) {
