...
### P7 — Schema versioning  · verdict CONFIRMED · effort XL · risk medium
...
### P8 — Legacy cleanup (LAST phase only)  · verdict CONFIRMED · effort M · risk high
Can be legacy after migration: `MF_Submissions.DataJson`, `MF_SubmissionValues`, `MF_WidgetData`, `MF_SearchIndex`. **Must NOT** be treated legacy: `MF_SavedDrafts.DataJson`, `MF_FormAnalytics.AggregatesJson`, workflow JSON columns. Verified: `MF_WidgetData`/`MF_SearchIndex` have **no C# runtime refs** and are **DNN-only** (absent from Oqtane/Umbraco/Web schema); still created by DNN install scripts (`SqlScripts` CREATE MF_WidgetData/MF_SearchIndex, manifest-registered `MegaForm.dnn:93`, duplicated `01_CreateTables.sql:267`) — but **`Uninstall.SqlDataProvider:47-48` already DROPs both**. Do NOT drop blindly in customer DBs; guard on a zero-row data check + release-note. **Do NOT drop DataJson or MF_SubmissionValues yet** (convert reports/search to typed first, or add a compat view).
