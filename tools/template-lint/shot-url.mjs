// Screenshot arbitrary URLs (full page). usage: node shot-url.mjs <outDir> <name=url> [name=url ...]
...
const jobs = process.argv.slice(3).map((s) => { const i = s.indexOf('='); return { name: s.slice(0, i), url: s.slice(i + 1) }; });
...
async function main() {
  const chrome = spawn(CHROME, [`--remote-debugging-port=${DBG}`, '--headless=new', '--disable-gpu', `--user-data-dir=${path.join(outDir, '.p')}`, '--window-size=1440,1000', '--hide-scrollbars', 'about:blank'], { stdio: 'ignore' });
  try {
    ...
    for (const j of jobs) {
      await cdp.call('Page.navigate', { url: j.url }); await sleep(7000);
      for (let i = 0; i < 40; i++) { if (await evalJs("!!document.querySelector('.mf-form-wrapper')")) break; await sleep(300); }
      await sleep(1500);
      const h = Math.min(Math.max(await evalJs('document.body.scrollHeight') || 1000, 900), 5000);
