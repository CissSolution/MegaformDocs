...
1. An employee (*jo.employee*) fills in the *Account setup* form and submits.
2. The workflow's first Approval step names the **Manager** role, so the task appears in
   *alex.manager*'s **My Inbox** — with the form name, step label, a due date from the step's SLA and
...
3. The moment step 1 completes, the workflow creates the **step 2** task for the **Finance**
   role: it is already waiting in *sam.finance*'s inbox. Finance approves the same way, and the
