...
Point the task at an endpoint you control — your ERP vendor's sandbox, a staging service of your
own, or a throwaway inspection URL — and make it answer the shape the response routes in §4 branch
on. For this walkthrough that is an order number plus a status, for example:

```json
{ "orderNo": "SO-10842", "status": "approved" }
```

Then submit the form once and follow it through:

1. The workflow reaches the API task and the request lands on your endpoint.
2. The reply's `status` decides the branch — return `approved` on one run and `needs_approval` on
   the next, and confirm the two runs take different paths.
3. The mapped values from the reply appear on the submission (§4).

> [!NOTE]
> An endpoint on loopback or a private network address is refused by the SSRF guard unless the host
> sets `MEGAFORM_ALLOW_PRIVATE_WEBHOOKS=1` — see
> [Calling a CRM on your own network](integration-webhook.md#7-calling-a-crm-that-is-on-your-own-network).
> This is the usual reason a first test looks like the task never fired.
