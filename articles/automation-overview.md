...
| Look up tax, exchange rate or shipping in real time | the lookup runs; writing the answer back into the submission needs PreInsert |
| Block a submission with a blacklist or fraud check | not configurable — needs PreInsert |
| Encrypt or normalise a field before it is stored | not configurable — needs PreInsert |
| Start an approval that routes itself | the workflow nodes do this with no code |
| Generate a PDF, Word or Excel document | not implemented |
| Move an uploaded file into a secure folder | not implemented |
| Publish an event to RabbitMQ, Kafka or SQS | not implemented |
