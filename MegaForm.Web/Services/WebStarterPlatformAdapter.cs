...
{
    /// <summary>
    /// [WebStarterPlatformAdapter v20260710] Standalone Web host implementation of
    /// IStarterPlatformAdapter. Lets the Core Business Starter services (LeaveRequest,
    /// Proposal, DocumentExchange, Recruitment, ConfiguredApp) run on the ASP.NET Core
    /// host without leaking platform-specific types into MegaForm.Core.
    /// </summary>
    public sealed class WebStarterPlatformAdapter : IStarterPlatformAdapter
    {
        private readonly MegaFormDbContext _db;

        public WebStarterPlatformAdapter(MegaFormDbContext db)
        {
        ...
        public int ResolveUserIdByNameOrEmail(string userName, string email)
        {
            ...
            var user = _db.WebUsers
                .Where(u => !u.IsDeleted)
                .AsEnumerable()
                .FirstOrDefault(u =>
                    (!string.IsNullOrWhiteSpace(normalizedUserName) && string.Equals(u.UserName, normalizedUserName, StringComparison.OrdinalIgnoreCase)) ||
                    (!string.IsNullOrWhiteSpace(normalizedEmail) && string.Equals(u.Email, normalizedEmail, StringComparison.OrdinalIgnoreCase)));
