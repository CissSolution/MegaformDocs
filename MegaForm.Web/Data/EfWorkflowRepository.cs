...
// ══════════════════════════════════════════════════════════════════════════════
//  EfWorkflowRepository
//  Stores WorkflowEnvelope (draft + applied) as JSON in MF_Forms.WorkflowJson.
//  Backward compatible: old plain WorkflowDefinition JSON is migrated on read.
// ══════════════════════════════════════════════════════════════════════════════
...
namespace MegaForm.Web.Data
{
    public class EfWorkflowRepository : IWorkflowRepository
    {
        ...
        public EfWorkflowRepository(MegaFormDbContext db) { _db = db; }

        // ─── Envelope ────────────────────────────────────────────────────────

        public WorkflowEnvelope GetEnvelope(int formId)
        {
            var form = _db.Forms.AsNoTracking()
        ...
        // ─── Legacy (engine uses applied def) ────────────────────────────────

        public WorkflowDefinition GetByFormId(int formId)
        {
            var env = GetEnvelope(formId);
            // Runtime must use only the last applied workflow.
            // Draft exists for the designer/editor and should not execute until Apply.
            return env.AppliedWorkflow;
        }

        public void Save(int formId, WorkflowDefinition definition)
        {
            // Legacy path: treat as both draft and applied (backward compat)
        ...
        {
            var row = _db.Set<WorkflowExecutionRow>()
                .FirstOrDefault(r => r.ExecutionId == ctx.ExecutionId);
            if (row == null) return;
        ...
        {
            var row = _db.Set<WorkflowExecutionRow>().AsNoTracking()
        ...
        public List<WorkflowExecutionSummary> ListExecutions(
            int formId, int pageIndex = 0, int pageSize = 20)
        {
            return _db.Set<WorkflowExecutionRow>().AsNoTracking()
        ...
        // ─── Cases & Tasks (human task runtime) ──────────────────────────────

        public WorkflowCaseInstance GetCase(string caseId)
        {
            var row = _db.Set<WorkflowCaseRow>().AsNoTracking()
                .FirstOrDefault(r => r.CaseId == caseId);
        ...
        public void SaveCase(WorkflowCaseInstance workflowCase)
        {
            ...
            var row = _db.Set<WorkflowCaseRow>().Find(workflowCase.CaseId);
            if (row == null)
            {
                row = new WorkflowCaseRow { CaseId = workflowCase.CaseId, CreatedAt = workflowCase.CreatedAt };
                _db.Set<WorkflowCaseRow>().Add(row);
            }
        ...
        {
            var row = _db.Set<WorkflowTaskRow>().AsNoTracking()
        ...
        {
            var q = _db.Set<WorkflowTaskRow>().AsNoTracking().AsQueryable();
        ...
        {
            if (task == null) return;
            var row = _db.Set<WorkflowTaskRow>().Find(task.TaskId);
            if (row == null)
            {
                row = new WorkflowTaskRow { TaskId = task.TaskId, CreatedAt = task.CreatedAt };
                _db.Set<WorkflowTaskRow>().Add(row);
            }
        ...
        public void AddTaskAction(WorkflowTaskAction action)
        {
            ...
            _db.Set<WorkflowTaskActionRow>().Add(new WorkflowTaskActionRow
            {
        ...
        {
            return _db.Set<WorkflowTaskActionRow>().AsNoTracking()
        ...
        {
            // e.g. "1.0.2-draft" -> "1.0.3-draft", or null -> "1.0.1-draft"
