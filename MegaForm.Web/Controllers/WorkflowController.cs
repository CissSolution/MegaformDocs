...
{
    /// <summary>
    /// [WebWorkflowTasks v20260710] Human-task workflow runtime endpoints for the
    /// standalone ASP.NET Core Web host. Mirrors the Oqtane MegaFormController
    /// Workflow* actions so the shared TS frontend works without branching.
    /// </summary>
    [ApiController]
    ...
    public class WorkflowController : ControllerBase
    {
        ...
        public IActionResult MyInbox(int recentCompleted = 25)
        {
            try
            {
                ...
                // [Submitter fix 2026-07-12] Twin of the Oqtane enrichment. The Web
                // host has no site user directory to join against, so the submitter
                // comes from the workflow case (GetOrCreateCase now records the real
                // actor name; legacy cases stored a numeric id and are skipped —
                // those keep the old "Unknown", no regression).
        ...
        public IActionResult SeedOrgDirectory()
        {
            try
            {
                ...
                foreach (var (dept, user, email, display) in org)
                {
                    if (depts.Add(dept))
                    {
                        _provisioning.EnsureRoleAsync(new MegaForm.Core.Workflow.WorkflowRoleProvisionRequest
                        {
                            PortalId = portalId,
                            Actor = actor,
                            RoleName = dept,
                    ...
                    var pu = _provisioning.EnsureUserAsync(new MegaForm.Core.Workflow.WorkflowUserProvisionRequest
                    {
                        PortalId = portalId,
                        Actor = actor,
                        UserName = user,
                    ...
                    _provisioning.AddUserToRoleAsync(new MegaForm.Core.Workflow.WorkflowUserRoleProvisionRequest
                    {
                        PortalId = portalId,
                        Actor = actor,
                        UserIdentifier = user,
