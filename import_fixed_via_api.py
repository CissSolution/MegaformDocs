#!/usr/bin/env python3
"""
Push fixed Premium JSON back into the existing test forms via the MegaForm API.
Reads qa_pages.json for module/form IDs and matches each page to a Premium-Fixed
JSON file by filename (case-insensitive).
"""
import json
import os
...
def build_payload(tpl, module_id, form_id, site_id=1):
    ...
    if "settings" not in tpl or not isinstance(tpl["settings"], dict):
        tpl["settings"] = settings

    # Prefer top-level values, fall back to settings.
    custom_html = str(tpl.get("customHtml", "") or settings.get("customHtml", "") or settings.get("CustomHtml", "") or "")
    top_css = str(tpl.get("customCss", "") or "")
    settings_css = str(settings.get("customCss", "") or settings.get("CustomCss", "") or "")
    combined_css = top_css
    if settings_css and settings_css not in top_css:
        combined_css = top_css + "\n\n" + settings_css
...
def find_fixed_file(page_path):
    segment = page_path.split("/")[-1].lower()
    candidates = [f for f in os.listdir(FIXED_DIR) if f.lower().endswith(".json")]
    for f in candidates:
...
def main():
    ...
    ok = 0
    for p in pages:
        filename = find_fixed_file(p["path"])
        if not filename:
            print(f"SKIP {p['path']}: no matching fixed template")
            continue
        status, body = import_one(filename, p["moduleId"], p["formId"])
        print(f"{filename}: status={status} body={body}")
        if status == 200:
            ok += 1
    print(f"\nImported {ok}/{len(pages)} forms.")
