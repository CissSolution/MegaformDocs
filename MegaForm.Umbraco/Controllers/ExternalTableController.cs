...
{
    /// <summary>
    /// Umbraco parity surface for external table binding.
    /// Mirrors MegaForm.Web.Controllers.ExternalTableController.
    /// Routes:
    ///   /umbraco/MegaForm/MegaFormApi/ExternalTable/...
    ///   /api/MegaFormPopup/ExternalTable/... (rewritten by MegaFormApiRouteRewriteMiddleware)
    /// </summary>
    [Authorize(Policy = "MegaFormBackOffice")]
    [Route("umbraco/MegaForm/MegaFormApi/ExternalTable")]
    public class ExternalTableController : ControllerBase
    {
        ...
        private List<string> AllowedConnections()
        {
            ...
            // admin-gated, so it carries appsettings-level trust. This is what makes a UI-added connection
            // reachable by the builder's table browser + capability probe.
            try
            {
        ...
        public IActionResult Bind([FromBody] BindBody body)
        {
            ...
            try
            {
                ...
                if (formId <= 0) return BadRequest(new { error = "could not save form" });
