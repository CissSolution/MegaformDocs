...
{
    /// <summary>
    /// Payment gateway proxy endpoints for Umbraco.
    /// Mirrors MegaForm.Web.Controllers.PaymentController.
    ...
    public class PaymentController : ControllerBase
    {
        ...
        public async Task<IActionResult> StripeCreateIntent([FromBody] JObject body)
        {
            try
            {
                ...
                {
                    new KeyValuePair<string, string>("amount", amountInt.ToString()),
                    new KeyValuePair<string, string>("currency", currency),
                    new KeyValuePair<string, string>("metadata[fieldKey]", fieldKey),
        ...
        // Mirrors MegaForm.Oqtane.Server PaymentController ([SecFix PAY-3]). The gateways
        // call these without any user auth — they authenticate via signature verification
        // inside PaymentWebhookService instead, which is strict and fail-closed.
        ...
        public async Task<IActionResult> StripeWebhook()
        {
            ...
            var result = _webhooks.HandleStripe(ResolveWebhookSiteId(), payload, signature);
        ...
        /// <summary>Umbraco currently runs as a single-site host; payment settings are stored
        /// under site 0 (ModuleSettingsPaymentGatewayStore). An explicit ?siteId= wins so a
        /// multi-site future does not need a code change.</summary>
        private int ResolveWebhookSiteId()
        {
            int siteId;
        ...
        /// <summary>Newtonsoft-serialized response — PaymentApiResult payloads carry
        /// Newtonsoft JTokens; never hand them to the host serializer via bare Ok().</summary>
