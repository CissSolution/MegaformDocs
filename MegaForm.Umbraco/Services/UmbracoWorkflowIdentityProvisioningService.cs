...
{
    /// <summary>
    /// Umbraco host implementation of workflow identity provisioning.
    /// Stores users/roles in custom MegaForm tables (MF_WebUsers / MF_WebRoles / MF_WebUserRoles)
    /// so the package stays lightweight and does not depend on Umbraco member management.
    /// </summary>
    public class UmbracoWorkflowIdentityProvisioningService : IWorkflowIdentityProvisioningService
    {
        private readonly MegaFormDbContext _db;
        private readonly IEmailSender _emailSender;
        private readonly ILogService _log;

        public UmbracoWorkflowIdentityProvisioningService(
        ...
        private static string BuildUserName(string requestedUserName, string email)
        {
            ...
            if (at > 0) return emailValue.Substring(0, at);
        ...
        private static string BuildPassword(WorkflowUserProvisionRequest request)
        {
            ...
            if (request?.GeneratePasswordIfEmpty == false) return string.Empty;
        ...
        {
            if (actor != null && !string.IsNullOrWhiteSpace(actor.UserName)) return actor.UserName;
            if (actor != null && actor.UserId > 0) return $"user:{actor.UserId}";
        ...
        private void TrySendWelcomeEmail(WorkflowProvisionedUser user)
        {
            ...
            {
                _log?.LogWarning("UmbracoWorkflowIdentityProvisioning", $"Failed to send welcome email to {user.Email}: {ex.Message}");
            }
        }
    }
}
