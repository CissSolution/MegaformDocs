# Event Registration Premium - MegaForm AI Design Contract

Source mock route: /forms/event-registration
Template slug: event-registration-premium
Root selector: `.mfp-event-registration-premium`

## Non-negotiable design rail

- Preserve `settings.customHtml` structure and `settings.customCss` scope selectors.
- Do not replace `.mfp-event-registration-premium`, wrapper class names, CSS variable names, hero image URLs, section containers, decorative/static table markup, dashed dividers, wave shape, or submit button class.
...
- Any AI edit must leave `settings.templateGuideSlug` pointing to `tpl-event-registration-premium`.
...
1. Inspect the form first and confirm it is custom-shell mode with `.mfp-event-registration-premium`.
...
4. If a color must change, append CSS variables or token overrides inside `.mfp-event-registration-premium` only; never replace the full CSS.
