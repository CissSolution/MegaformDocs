# Insurance Enrollment - MegaForm AI Design Contract

Source mock route: /forms/insurance-enrollment
Template slug: insurance-enrollment
Root selector: `.mfp-insurance-enrollment`

## Non-negotiable design rail

- Preserve `settings.customHtml` structure and `settings.customCss` scope selectors.
- Do not replace `.mfp-insurance-enrollment`, wrapper class names, CSS variable names, hero image URLs, section containers, decorative/static table markup, dashed dividers, wave shape, or submit button class.
...
- Any AI edit must leave `settings.templateGuideSlug` pointing to `tpl-insurance-enrollment`.
...
1. Inspect the form first and confirm it is custom-shell mode with `.mfp-insurance-enrollment`.
...
4. If a color must change, append CSS variables or token overrides inside `.mfp-insurance-enrollment` only; never replace the full CSS.
