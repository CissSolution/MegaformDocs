// MegaForm Razor Widget — Phase 0 stub service implementations
// ──────────────────────────────────────────────────────────────────────
...
// AsyncLocal request state instead of holding their own state.
//
// Phase 1 will replace these stubs with full implementations:
//   - IMfFormContext  → read live values from the in-circuit form scope
//   - IMfUserContext  → resolved from Oqtane authn / DNN portal user
//   - IMfSiteContext  → site settings + connection registry
//   - IMfSqlExecutor  → wraps IConnectionRegistry + Dapper
//   - IMfRazorEmitter → posts back to the form's JS bridge
using System;
...
namespace MegaForm.Oqtane.Server.Services
{
    ...
    public class StubUserContext : IMfUserContext
    {
        ...
        public bool IsInRole(string roleName) => Array.IndexOf(((IReadOnlyList<string>)Roles).ToArray(), roleName) >= 0;
    }
    ...
    public class RegistrySqlExecutor : IMfSqlExecutor
    {
        ...
        private static readonly System.Text.RegularExpressions.Regex _paramRx =
        ...
        private IEnumerable<dynamic> Execute(string sql, object parameters, string connectionKey, bool isScalar, out object scalar)
        {
            ...
            foreach (System.Text.RegularExpressions.Match m in _paramRx.Matches(sql))
            {
    ...
    // Lightweight static helper used inside System.Linq.Enumerable for Array.IndexOf
    internal static class LocalLinqExt
    {
