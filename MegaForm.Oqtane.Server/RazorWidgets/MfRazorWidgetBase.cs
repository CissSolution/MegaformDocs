...
// ──────────────────────────────────────────────────────────────────────
// Lives in the platform-neutral `MegaForm.Razor` namespace even though
// the runtime is currently the Oqtane companion service. This is so
// customer-authored .razor sources (whether the host is DNN or Oqtane)
// reference a name that does not lie about the platform. Backward-
// compatible shim under `MegaForm.Oqtane.Server.RazorWidgets` below
// keeps older sources / 3rd-party tools compiling.
...
namespace MegaForm.Razor
{
    public abstract class MfRazorWidgetBase : ComponentBase
    {
        ...
        // resolve it via @inherits — that path lands in a future block.
        //
        // Usage:
        //   @(await RenderPartial("kpi-card", new { title = "Sales", value = "$5,420" }))
        //
        // Partial caching: 30s in-memory per-process. Restart Oqtane.Server
        // or POST /AiTools/Knowledge to invalidate sooner.
            ...
            if (body == null)
            {
                try
                {
                    var rows = await Sql.QueryAsync(
                        "SELECT TOP 1 Body FROM dbo.MF_AI_Knowledge WHERE Slug = @slug AND Kind = 'razor_partial'",
                        new { slug });
                    if (rows != null)
                    {
...
// Backward-compat shim — customer sources written before the namespace
// rename keep working. Will print a one-line build-time hint when AI
// emits the old `@using` (no code change required from the customer).
namespace MegaForm.Oqtane.Server.RazorWidgets
{
    /// <summary>
    /// DEPRECATED: use MegaForm.Razor.MfRazorWidgetBase. Kept for
    /// backward compatibility with sources written before 2026-05-31.
    /// </summary>
    public abstract class MfRazorWidgetBase : MegaForm.Razor.MfRazorWidgetBase { }
}
