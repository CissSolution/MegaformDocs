...
function systemPrompt(): string {
  ...
  return [
    ...
    // [v20260812-KbPerTemplate] Was "the KB has 178 templates" with a hard-coded `tpl-` slug.
    // Both became false: template knowledge now arrives PER TEMPLATE when one is installed from
    // the online gallery (slug `gallery-<slug>`), while templates bundled in the package keep
    // `tpl-<slug>`. Quoting a count made the model insist templates existed on a site that had
    // none; quoting a prefix made it guess a slug instead of using the one it was just handed.
    '- ⚠ TOP RULE — When the user asks to CREATE / MAKE / BUILD a new form ("tao 1 form X", "create a booking form", "I need a job application form"), DO NOT hand-roll add_field ops. INSTEAD: (1) call `list_knowledge(kind="form_template", search="<keywords>")` — production-quality templates covering booking / contact / order / payment / HR / education / healthcare / real-estate / survey / application and more. (2) Pick the best match by title + summary + tags. (3) Call `get_knowledge(slug="<the slug EXACTLY as returned by list_knowledge>")` — never construct the slug yourself — and the `examples` field contains a `replace_form_schema` op you can apply VERBATIM. (4) Emit that op. (5) THEN customize via set_field_property / add_field / remove_field. This shortcut turns a 30-op chain into 1 op + 2-3 tweaks, and the resulting form is automatically production-quality (proper Row/Section structure, sensible field order, real labels, correct cascade chains). If list_knowledge returns NOTHING for the topic, say so and offer to design from scratch — the site may simply not have that template installed yet (premium designs bring their own knowledge when installed from the Template Gallery). Do NOT keep re-querying with different keywords more than twice.',
    ...
    while (iter < MAX_TOOL_ITERATIONS) {
      ...
      for (const call of calls) {
        ...
        (opts.history as ChatMessageWithTools[]).push({
          ...
          content: '',
          toolCallId: call.id,
          // [KbFetchBudget v20260812] The tool NAME decides the size budget — a knowledge
          // document fetched by slug is allowed to arrive whole (see LARGE_RESULT_TOOLS).
          toolResult: serializeToolResult(toolResult, call.name),
        });
      }
    }
    thinking.remove();
