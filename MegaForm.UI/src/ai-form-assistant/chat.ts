...
function systemPrompt(): string {
  ...
  return [
    ...
    '- ⚠ TOP RULE — NEVER write a raw image URL into htmlContent, defaultValue, or widgetProps. The AI has a 100% hallucination rate on `https://images.unsplash.com/photo-<random>` and `https://source.unsplash.com/random/...` URLs — they 404 and render a broken-image icon. Do ONE of these instead: (1) Call the `get_safe_image_url(keywords, width?, height?, style?: "photo"|"placeholder")` tool — it returns a guaranteed-working URL on picsum.photos (seed-based, deterministic, always loads) or placehold.co. PASTE THAT URL into your <img src="…">; or (2) Emit `set_field_image_unsplash` op with target:"htmlContent" — it writes the entire <img> tag for you. Allowlisted hosts for raw URLs: ONLY picsum.photos, placehold.co, data: URIs, relative/same-origin paths. `images.unsplash.com` is NOT allowed (you cannot guess a valid photo-<32hex> id). `source.unsplash.com` is deprecated and NOT allowed. The dispatcher REJECTS [IMG-001] any Html field whose htmlContent contains an <img src="…"> outside this allowlist.',
    ...
    '- Images: use set_field_image_unsplash (do not hand-write image URLs — you will hallucinate broken ones).',
...
function appendStagedOpsCard(log: HTMLElement, ops: Op[]): void {
  const summary = ops.map((o, i) => {
    ...
    else if (o.op === 'set_field_image_unsplash') detail = String(o.key || '') + ' ← unsplash:"' + String(o.query || '') + '"';
