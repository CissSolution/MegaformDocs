...
import { enrichRichChoiceOptionsFromCatalog, replaceInventedImageUrlsWithCatalog } from '@shared/rich-choice-catalog';
...
const AI_SYSTEM_PROMPT = [
  ...
  '  Text · Email · CompositePhone · Number · Date · Time · Textarea · Select · Radio · Checkbox · Chips · Cards · File · Rating · Signature · Url · Hidden · Section · Row',
  '  CHOICE STYLES — DEFAULT to rich choices: single-choice with <=6 short options -> Cards; multi-choice tags/interests/skills/preferences -> Chips. Fall back to plain Radio/Checkbox only for >8 options or long sentence labels. You may instead use Radio/Checkbox + properties.optionDisplay:"cards"|"chips" — identical look and the premium templates use this pattern. Chips & Cards BOTH need an "options":[{value,label}] array. Cards options SHOULD include a short helpful description when you can write one; add badge/meta only when useful.',
  '  CARD/CHIP DESIGN RAILS — DO NOT invent CSS, emoji icons, FontAwesome icon names, iconHtml, SVG, or image URLs for choice options. Emit the option text/values/descriptions only. MegaForm deterministically assigns icons from the bundled mock source catalog (rich-selection-controls plus premium form icons such as ticket, users, wine, pizza, cake, calendar, map-pin) and applies the global rich-choice skin.',
  'Each field: { "key":"snake_case", "type":"<Type>", "label":"<Display>", "required":<bool>, "placeholder":"…"? , "options":[{value,label}]?, "properties":{"optionDisplay":"cards|chips"}? }',
  '',
  'LAYOUT RULES:',
  ...
  '  9. Images: use ONLY MegaForm mock-catalog relative URLs such as `/Modules/MegaForm/img/mock/event-hero.png`, `/Modules/MegaForm/img/mock/festa-italiana-hero.png`, `/Modules/MegaForm/img/mock/australia-hero.png`, `/Modules/MegaForm/img/mock/party-hero.png`. NEVER use picsum, unsplash, source.unsplash, external image URLs, invented filenames, or data scraped from memory. The container CSS MUST constrain dimensions (e.g. `.mfp-hero-banner{height:280px}` or `.mfp-hero{aspect-ratio:3/4}`) so an `<img>` cannot blow up to its natural size and dominate the page. Set `.mfp-hero-img{width:100%;height:100%;object-fit:cover}` ALWAYS.',
  '  10. Field stack spacing in every custom shell MUST use `gap:var(--mf-field-gap,18px)` on `.mfp-form-inner`, `.mfp-card-inner`, or `.mfp-body-inner` so Theme Designer can tune vertical rhythm. Do not hardcode 16px/18px gaps on the field stack.',
  '  11. 🛑 MULTI-STEP + CUSTOM-SHELL ARE INCOMPATIBLE. If the user asks for a multi-step / wizard form, you MUST use plain Section fields with `properties.pageBreak:true` and NO customHtml. The renderer builds step UI + Next/Previous buttons automatically. Hand-rolled step indicators inside customHtml are visual-only — they do NOT page through fields, so step 1 ends up empty and all fields land on whatever step the customHtml lists them in. PICK ONE: (a) Multi-step wizard → plain layout, `settings.multiPage:true` + Section.pageBreak, no customHtml; (b) Premium custom-shell → single-page layout, no step indicators in customHtml.',
  '',
...
async function callAI(userText: string, history: any[], attachments?: any[], selectedTables?: string[], builderForm?: any): Promise<{ schema?: any; explain?: string; rawText?: string }> {
  ...
  if (obj && obj.schema && typeof obj.schema === 'object' && Array.isArray(obj.schema.fields)) {
    ...
    if (!isPremiumEdit) {
      normalizeRichChoiceFieldsDeep(obj.schema, 0);
      normalizeCatalogImageUrlsDeep(obj.schema, [prompt, obj.schema.title, obj.schema.description].map((v) => String(v || '')).join(' '), 0);
    }
    if (isPremiumEdit) {
...
const RICH_CARD_RE = /\b(plan|pricing|price|tier|package|pass|ticket|membership|subscription|option\s+picker|service\s+level)\b/i;
const RICH_CHIP_RE = /\b(interests?|hobbies|skills?|tags?|topics?|features?|preferences?|services?\s+(needed|required|wanted)|products?\s+(interested|wanted))\b/i;
const SINGLE_CHECK_RE = /\b(agree|terms?|privacy|consent|confirm|subscribe|opt[-\s]?in)\b/i;
function optionLabelsAreShort(options: any[]): boolean {
  return (options || []).every((opt) => {
    const label = String((opt && (opt.label || opt.title || opt.value)) || '').trim();
    return !!label && label.length <= 42 && !/[.!?]\s/.test(label);
  });
}
function setOptionDisplay(field: any, display: 'cards' | 'chips'): void {
  field.optionDisplay = display;
  field.properties = (field.properties && typeof field.properties === 'object') ? field.properties : {};
  field.properties.optionDisplay = display;
  delete field.optionColumns;
  delete field.properties.optionColumns;
}
// Deterministic AI-output cleanup for the default/premium-create paths. The renderer
// already supports these skins; this keeps weak model responses from falling back to
// plain Radio/Checkbox for the most common premium choice controls.
function normalizeRichChoiceFieldsDeep(node: any, depth: number): void {
  if (!node || typeof node !== 'object' || depth > 8) return;
  if (Array.isArray(node)) { node.forEach((v) => normalizeRichChoiceFieldsDeep(v, depth + 1)); return; }
  const options = Array.isArray(node.options) ? node.options : [];
  if (options.length > 1 && optionLabelsAreShort(options)) {
    const t = String(node.type || '').toLowerCase().replace(/[\s_-]+/g, '');
    const text = [node.key, node.label, node.name, node.placeholder].map((v) => String(v || '')).join(' ');
    if (options.length <= 6 && RICH_CARD_RE.test(text) && (t === 'radio' || t === 'select' || t === 'cards')) {
      node.type = 'Cards';
      setOptionDisplay(node, 'cards');
    } else if (options.length <= 12 && RICH_CHIP_RE.test(text) && !SINGLE_CHECK_RE.test(text) && (t === 'checkbox' || t === 'select' || t === 'chips')) {
      node.type = 'Chips';
      setOptionDisplay(node, 'chips');
    }
    const display = String(node.optionDisplay || node.properties?.optionDisplay || '').toLowerCase();
    if (node.type === 'Cards' || display === 'cards') enrichRichChoiceOptionsFromCatalog(node, 'cards');
    if (node.type === 'Chips' || display === 'chips') enrichRichChoiceOptionsFromCatalog(node, 'chips');
  }
  for (const k of Object.keys(node)) {
    const v = node[k];
    if (v && typeof v === 'object') normalizeRichChoiceFieldsDeep(v, depth + 1);
  }
}

function normalizeCatalogImageUrlsDeep(node: any, contextText: string, depth: number): void {
  if (!node || typeof node !== 'object' || depth > 8) return;
...
const PURE_GRID_SHELL_CSS =
  ...
  '.mfp.mfp-pure-grid .mfp-card-body{padding:28px 32px 28px;display:flex;flex-direction:column;gap:var(--mf-field-gap,18px)}' +
  '.mfp.mfp-pure-grid .mfp-section{display:flex;flex-direction:column;gap:var(--mf-field-gap,18px)}' +
