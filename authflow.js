...
export class UmbAuthFlow {
    ...
    #link_key_endpoint;
    #unlink_endpoint;
    constructor(openIdConnectUrl, redirectUri, postLogoutRedirectUri, timeoutSignal, clientId = 'umbraco-back-office', scope = 'offline_access') {
        /**
         ...
         */
        this.authorizationSignal = new Subject();
     ...
     */
    makeAuthorizationRequest(identityProvider, usernameHint) {
    ...
    async signOut() {
        ...
        if (this.#tokenResponse) {
            ...
            if (this.#tokenResponse.refreshToken) {
                ...
                signOutPromises.push(this.#tokenHandler.performRevokeTokenRequest(this.#configuration, refreshTokenRevokeRequest));
            }
        }
     ...
     */
    async unlinkLogin(loginProvider, providerKey) {
     ...
     */
    async #makeTokenRequest(code, codeVerifier) {
        const extras = {};
        if (codeVerifier) {
     ...
     */
    async #performTokenRequest(request) {
        try {
